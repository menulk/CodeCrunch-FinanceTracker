...
// GetTotalAmountByCategory returns the total amount spent per category.
func (db *DB) GetTotalAmountByCategory() (map[string]float64, error) {
	rows, err := db.db.Query("SELECT category, SUM(amount) FROM expenses GROUP BY category")
	if err != nil {
	...
	for rows.Next() {
		...
		if err != nil {
			logger.Error(fmt.Errorf("Could not scan row: %w", err).Error())
			continue
		}
