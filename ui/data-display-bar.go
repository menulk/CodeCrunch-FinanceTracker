...
type DataDisplay struct {
	...
	MeanExpenseLabel   material.LabelStyle
	StdDevExpenseLabel material.LabelStyle
}
...
func (d *DataDisplay) Update() {
	...
	d.MeanExpenseLabel.Text = fmt.Sprintf("MeanExpense: %.2f", d.monthData.MeanExpense)
	d.StdDevExpenseLabel.Text = fmt.Sprintf("StandardDeviationExpense: %.2f", d.monthData.StandardDeviationExpense)
}
...
func (d *DataDisplay) Layout(gtx layout.Context) layout.Dimensions {
	if d.state == Editing {
		return layout.Flex{
			...
			layout.Rigid(
				d.MeanExpenseLabel.Layout,
			),
			layout.Rigid(
				d.StdDevExpenseLabel.Layout,
			),
		)
...
func createDataDisplay(th *material.Theme, controller domain.API, monthData *domain.MonthData) DataDisplay {
	...
	MeanExpenseLabel := material.Label(th, unit.Sp(16), fmt.Sprintf("MeanExpense: %.2f", 0.00))
	StdDevExpenseLabel := material.Label(th, unit.Sp(16), fmt.Sprintf("StandardDeviationExpense: %.2f", 0.00))
	...
	MeanExpenseLabel.MaxLines = 1
	StdDevExpenseLabel.MaxLines = 1
	...
	return DataDisplay{
		...
		MeanExpenseLabel:   MeanExpenseLabel,
		StdDevExpenseLabel: StdDevExpenseLabel,
